 ...
 *
 * Fase 4 update (2026-04-25): identiteitsherkenning via auth.uid().
 * setAuthInBrowser() zet de Supabase auth-sessie in localStorage zodat
 * de app de deelnemer herkent via supabase.auth.getUser().
 */
...
test.describe('PW-1: Happy path storten', () => {
  ...
  async function navigeerNaarStorten(page) {
    await page.goto('/')
  ...
  test('PW-1a: snelknop €10 → storten → saldo zichtbaar op overzicht', async ({ page }) => {
    await navigeerNaarStorten(page)
